...
def parser(courses: list) -> None:
    ...
    for course in tqdm(courses, desc='Обработка курсов'):
        ...
        driver.find_element(by=By.XPATH, value='//*[@id="nav628757783"]/div/div/nav/ul/li[1]/a').click()
        driver.find_element(by=By.CLASS_NAME, value='search-form__input ').send_keys(f'{course}\n')
        time.sleep(5)
        for k in range(5):
        ...
        for number in trange(length, desc=f'Загружаем карточки для курса {course}'):
            cart_2 = driver.find_elements(by=By.CLASS_NAME, value='catalog-rich-card__link-wrapper')
            try:
                name = cartochki[number].find_element(by=By.CLASS_NAME, value='course-card__title')
                name_csv.append(name.text)
            except:
                name_csv.append(None)
            try:
                author = cartochki[number].find_element(by=By.CLASS_NAME, value='course-card__author')
                authors_csv.append(author.text)
            except:
                authors_csv.append(None)
            try:
                sum = cartochki[number].find_element(by=By.CLASS_NAME, value='course-card__summary')
                summary_csv.append(sum.text)
            except:
                summary_csv.append(None)
            try:
                expense = cartochki[number].find_element(by=By.CLASS_NAME, value='course-card__price')
            ...
            try:
                ...
                flag_1 = True
                flag_2 = True
                information = cartochki[number].find_elements(by=By.CLASS_NAME, value='course-card__widget')
                for info in information:
                    ...
                    if 'learners' in label:
                        ...
                        if 'K' in participation:
                            participation = float(participation.replace(' ', '').replace('K', '')) * 1000
                        learners.append(int(participation))
                        flag_1 = False
                    if 'workload' in label:
                        index = label.find('ч')
                        workload.append(int(label[index - 3:index].replace(' ', '')))
                        flag_2 = False
                if flag_0:
        ...
        if i == 0:
            ...
            all_csv = pd.concat([all_csv, pd.DataFrame(d_csv)], ignore_index=True)
